...
import { Waves, Activity, Droplets, ShieldAlert, Radio, Users } from 'lucide-react';
...
export default function AquiferSpectrogramPanel({ onClose }: { onClose?: () => void }) {
  const [activeTab, setActiveTab] = useState<'aquifer' | 'sessions'>('aquifer');
  ...
  return (
    <div className="bg-[#0a0a0c] border border-cyan-500/30 rounded-xl p-4 flex flex-col gap-4 relative overflow-hidden h-full">
        ...
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-mono text-sm uppercase tracking-widest text-cyan-400 font-bold">
                ARKHE-Ω Observability
              </h2>
              <div className="text-[10px] font-mono text-cyan-500/70 uppercase">
                {activeTab === 'aquifer' ? 'Aquifer Spectrogram & ZK Water Balance' : 'Lucent-Ω User Session Analysis'}
              </div>
            </div>
          </div>
          ...
          <div className="flex bg-black/40 p-1 rounded-lg border border-cyan-500/20">
            <button
              ...
              className={`px-3 py-1.5 rounded font-mono text-[10px] uppercase tracking-wider transition-all flex items-center gap-2 ${activeTab === 'aquifer' ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30' : 'text-cyan-500/40 hover:text-cyan-400'}`}
            >
              <Waves className="w-3 h-3" /> Aquifero
            </button>
            <button
              onClick={() => setActiveTab('sessions')}
              className={`px-3 py-1.5 rounded font-mono text-[10px] uppercase tracking-wider transition-all flex items-center gap-2 ${activeTab === 'sessions' ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30' : 'text-cyan-500/40 hover:text-cyan-400'}`}
            >
              <Users className="w-3 h-3" /> Sessões Lucent
            </button>
          </div>
        </div>
      ...
      <div className="flex-1 min-h-0 relative z-10">
        {activeTab === 'aquifer' ? (
          <div className="flex flex-col gap-4 h-full">
            ...
            <div className="flex gap-4 justify-between mt-auto border-t border-cyan-500/10 pt-3">
              <button
                onClick={toggleAudio}
