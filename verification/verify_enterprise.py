...
def verify_enterprise_plus(page):
    ...
    # 5. Acionar o subagente S3 (Phylax) - Frustration Proof
    s3_button = page.get_by_role("button", name="ACIONAR Phylax POC")
    expect(s3_button).to_be_visible()
    s3_button.click()

    # 6. Abrir painel de Slashing
    slashing_trigger = page.get_by_title("Conselho de Decoerência (Slashing)")
